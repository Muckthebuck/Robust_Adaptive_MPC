...
class Draw_MPC_Obstacle(object):
    def __init__(self, robot_states: list, init_state: np.array, target_state: np.array, obstacle: np.array,
                 tube_size: float = 0.1, rob_diam=0.3, export_fig=False):
        self.robot_states = robot_states
        self.init_state = init_state
        self.target_state = target_state
        self.rob_radius = rob_diam / 2.0
        ...
        if obstacle is not None:
            ...
            print('No obstacle given, breaking.')
        self.tube_upper = tube_size
        self.tube_lower = -tube_size
        self.fig.set_size_inches(7, 6.5)
        # init for plot
        self.animation_init()
    ...
    def animation_init(self):
        ...
        # Plot initial state of the robot
        self.robot_body = plt.Circle(self.init_state[:2], self.rob_radius, color='r', fill=False)
        self.ax.add_artist(self.robot_body)
        self.robot_arr = mpatches.Arrow(self.init_state[0], self.init_state[1],
                                        self.rob_radius * np.cos(self.init_state[2]),
        ...
        # Plot tube around predicted trajectory
        self.tube_circle = plt.Circle(self.init_state[:2], self.tube_upper, color='orange', alpha=0.3)
        self.ax.add_artist(self.tube_circle)

        return self.target_circle, self.target_arr, self.robot_body, self.robot_arr, self.obstacle_circle, self.tube_circle
    ...
    def animation_loop(self, indx):
        ...
        self.tube_circle.center = position
        self.tube_circle.set_radius(self.tube_upper)

        return self.robot_arr, self.robot_body, self.tube_circle
